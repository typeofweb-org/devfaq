 ...
 */
{
  const backupChanged = danger.git.modified_files.includes('src/seeders/backup.sql');

  if (backupChanged) {
    fail(
      `Don't edit backup.sql! Create a migration script instead.`,
      'src/seeders/backup.sql'
    );
  }
}
 ...
 */
{
  const migrationsChanged = danger.git.modified_files.find(file => file.includes('src/migrations'));
 ...
 */
{
  ...
  const migrationsAdded = danger.git.created_files.find(file => file.includes('src/migrations'));
...
/**
 * Prevent modifying seeders
 */
{
  const seedersChanged = danger.git.modified_files.find(file => file.includes('src/seeders'));
  if (seedersChanged) {
    fail(
      `Don't edit existing seeders files! Create a new migration instead.`,
      seedersChanged
    );
  }
}
