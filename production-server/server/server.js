...
function getPathForStaticResource(pathname) {
    if (favicons.includes(pathname)) {
        return path_1.join(__dirname, 'static', 'favicons', pathname);
    }
    else if (staticFiles.includes(pathname)) {
        return path_1.join(__dirname, 'static', pathname);
    }
    else if (pathname === '/robots.txt') {
        return path_1.join(__dirname, 'static', isDev ? 'robots.dev.txt' : 'robots.prod.txt');
    }
    return undefined;
}
