 ...
 */
{
  const backupChanged = danger.git.modified_files.includes('src/migrations/backup.sql');

  if (backupChanged) {
    fail(`Don't edit backup.sql! Create a migration script instead.`, 'src/migrations/backup.sql');
  }
}
